...
static void mc_cn_work(struct work_struct *work)
{
	...
	callback->out = callback->callback(msg, parms);
	complete(&entry->comp);
		    ...
		    struct netlink_skb_parms *))
{
	...
	entry = kmem_cache_alloc(cn_cachep, GFP_KERNEL);
	if (!entry) {
		ret = -ENOMEM;
		goto out;
	}
	...
	entry->callback.callback = callback;
...
static int __init connector_init(void)
{
	...
	cn.queue->workqueue = create_singlethread_workqueue("mc_connector");
...
static __exit void connector_exit(void)
{
	struct cn_entry *pos, *n;
	struct cn_queue *queue = cn.queue;
...
module_init(connector_init);
module_exit(connector_exit);

MODULE_AUTHOR("Li Jianguo <byjgli@gmail.com>");
MODULE_DESCRIPTION("transport helper for kmemcache & umemcached");
