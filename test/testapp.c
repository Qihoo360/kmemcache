...
static bool safe_strtoull(const char *str, uint64_t *out)
{
	...
	ull.str.p   = str;
	ull.str.len = strlen(str) + 1;

	if (!ioctl(kmcfd, KMC_STR_ULL, &ull)) {
		*out = ull.out;
		return true;
	}
	return false;
...
static bool safe_strtoll(const char *str, int64_t *out)
{
	...
	ll.str.p   = str;
	ll.str.len = strlen(str) + 1;

	if (!ioctl(kmcfd, KMC_STR_LL, &ll)) {
		*out = ll.out;
		return true;
	}
	return false;
...
static bool safe_strtoul(const char *str, uint32_t *out)
{
	...
	ul.str.p   = str;
	ul.str.len = strlen(str) + 1;

	if (!ioctl(kmcfd, KMC_STR_UL, &ul)) {
		*out = ul.out;
		return true;
	}
	return false;
...
static bool safe_strtol(const char *str, int32_t *out)
{
	...
	l.str.p   = str;
	l.str.len = strlen(str) + 1;

	if (!ioctl(kmcfd, KMC_STR_L, &l)) {
		*out = l.out;
		return true;
	}
	return false;
 ...
 */
static pid_t start_server(in_port_t *port_out, int timeout) {
    char environment[80];
...
static enum test_return start_memcached_server(void) {
    server_pid = start_server(&port, 600);
    sock = connect_server("127.0.0.1", port, false);
    return TEST_PASS;
}
...
static enum test_return test_issue_101(void) {
    ...
    server_pid = start_server(&port, 1000);
...
int main(int argc, char **argv)
{
    ...
    kmcfd = open(KMC_DEVICE, O_RDWR);
    if (kmcfd == -1) {
	    perror("open kmemcache device error\n");
	    return 0;
    }
